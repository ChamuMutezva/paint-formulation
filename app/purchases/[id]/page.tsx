...
import { type Purchase, type Customer, type Paint, type Formulation, scaleFormulation } from "@/lib/db"
...
export default async function PurchaseDetailPage({ params }: { params: { id: string } }) {
  ...
  const scaledFormulations = formulations.map((f) => scaleFormulation(f, purchase.size, paint.base_size))
      ...
      <header className="border-b border-border">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">Purchase Details</h1>
              <p className="text-muted-foreground mt-1">View purchase and formulation information</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-lg font-medium">{formatDate(purchase.purchase_date)}</p>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-lg font-medium">{paint.color_name}</p>
              </div>

              <div>
                <Label>Product Type</Label>
                <p className="text-lg font-medium">{paint.product_type}</p>
              </div>

              {paint.description && (
                <div>
                  <Label>Description</Label>
                  <p className="text-sm text-muted-foreground mt-1">{paint.description}</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              Scaled from base formula ({paint.base_size} {paint.base_unit})
            </CardDescription>
          </CardHeader>
          <CardContent>
            {scaledFormulations.length === 0 ? (
              <p className="text-muted-foreground italic">No formulation available for this paint</p>
            ) : (
                ...
                <div className="grid gap-3">
                  {scaledFormulations.map((formulation, idx) => (
                    <div key={idx} className="flex justify-between items-center p-4 bg-muted rounded-lg">
                      <span className="font-medium">{formulation.component_name}</span>
