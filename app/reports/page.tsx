...
import { ArrowLeft, TrendingUp, Users, Palette, ShoppingCart } from "lucide-react"
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">Business insights and trends</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-3 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{data.totalPaints}</div>
              ...
              <div className="space-y-3">
                {data.monthlyTrend.map((month: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-4">
                    <div className="w-32 text-sm font-medium">{formatMonth(month.month)}</div>
                    <div className="flex-1">
                      <div className="h-8 bg-primary/20 rounded-lg relative overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.max(
                              (Number(month.purchase_count) /
                                Math.max(...data.monthlyTrend.map((m: any) => Number(m.purchase_count)))) *
                                100,
                              10,
                            )}%`,
                          }}
                        >
                          <span className="text-sm font-semibold text-primary-foreground">{month.purchase_count}</span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Top selling paint colors</CardDescription>
            </CardHeader>
            <CardContent>
              {data.popularColors.length === 0 ? (
                ...
                <div className="space-y-4">
                  {data.popularColors.map((color: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">{color.product_type}</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-semibold">{color.purchase_count} sales</div>
                        <div className="text-sm text-muted-foreground">
                          {Number(color.total_volume).toFixed(1)}L total
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Top customers by purchase count</CardDescription>
            </CardHeader>
            <CardContent>
              {data.activeCustomers.length === 0 ? (
                ...
                <div className="space-y-4">
                  {data.activeCustomers.map((customer: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Badge variant="secondary" className="w-8 h-8 rounded-full flex items-center justify-center">
                          {idx + 1}
                      ...
                      <div className="text-right">
                        <div className="font-semibold">{customer.purchase_count} purchases</div>
                        <div className="text-sm text-muted-foreground">Last: {formatDate(customer.last_purchase)}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <Card className="mt-8">
          ...
          <CardContent>
            {data.recentPurchases.length === 0 ? (
              <p className="text-muted-foreground text-center py-8">No recent purchases</p>
            ) : (
              <div className="space-y-3">
                {data.recentPurchases.map((purchase: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-muted-foreground">
                        {purchase.color_name} - {purchase.product_type}
                      </div>
                    </div>
