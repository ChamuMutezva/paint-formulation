...
export function ColorSearchResults({ results }: ColorSearchResultsProps) {
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          <p className="text-muted-foreground">No paint color found with that name</p>
        </CardContent>
      </Card>
    )
  }

  const totalQuantity = baseFormulations.reduce((sum, f) => sum + f.quantity, 0)
  ...
  return (
    <div className="space-y-4">
          ...
          {showFormulation && (
            <div className="space-y-2 p-3 bg-muted rounded-lg">
              <div className="font-medium text-sm mb-2">
                Base formula ({paint.base_size} {paint.base_unit}):
              </div>
              {baseFormulations.length === 0 ? (
                <p className="text-sm text-muted-foreground italic">No formulation available</p>
              ) : (
                <>
                  {baseFormulations.map((formulation: any, idx: number) => (
      ...
      <div>
        <h3 className="text-lg font-semibold mb-3">Customers Who Purchased This Color</h3>
        {customers.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center">
              <p className="text-muted-foreground">No customers have purchased this color yet</p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-muted-foreground">
                        {formatDate(customer.purchase_date)} • {customer.size} {customer.unit}
                      </div>
                      {customer.notes && <div className="text-sm text-muted-foreground mt-1">{customer.notes}</div>}
                    </div>
                    <Link href={`/purchases/${customer.purchase_id}`}>
